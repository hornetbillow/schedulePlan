...
namespace Raise.Monitor {
    public partial class RuleConfigEditFrm : BaseFrm {
        public RuleConfigEditFrm() {
            InitializeComponent();
        }

        public void SetObjectToEdit(int id) {
            var ruleConfig = ServiceUtils.GetConnection.QueryFirst<RuleConfig>("Select * From RuleConfig where Id = :id", new { id });
            if(ruleConfig == null)
                return;
            this.cbIsOData.Checked = ruleConfig.IsAuthentication == (int)YesOrNo.是;
            ...
            this.cmbContentType.Text = ruleConfig.ContentType;
            this.txbCron.Text = ruleConfig.Cron;
            this.rbWebService.Checked = ruleConfig.IsWebService == (int)YesOrNo.是;
            this.rbRestful.Checked = ruleConfig.IsWebService != (int)YesOrNo.是;
