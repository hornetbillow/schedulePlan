...
namespace Raise.Monitor {
    public partial class CronExpressionFrm : BaseFrm {
        ...
        private void ReloadRunDetail(string cronStr) {
            ...
            try {
                var timeList = Utils.GetTaskeFireTime(cronStr);
        ...
        private void ReverseUi(List<string> crons) {
            ...
            for(var i = 0; i < crons.Count; i++) {
                ...
                if(crons[i].Contains(",") || Utils.IsInt(crons[i])) {
                ...
                if(crons[i].Contains("L")) {
                    ...
                    foreach(Control control in tabControls) {
                        ...
                        if(control.Name.Contains("nud")) {
                            var nud = (NumericUpDown)control;
                            nud.Value = Utils.IsInt(chars[0].ToString()) ? Convert.ToDecimal(chars[0].ToString()) : 1;
                        } else {
